...
run_name = "fader_gan_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
# Model stuff
model = FaderNetworkGAN(n_channels=n_channels, n_latent=n_latent, n_c=[n_clim, n_lc], normalization="MCBN").to(device)
...
optim_generation = torch.optim.Adam(nn.ModuleList((model.encoder, model.decoder)).parameters(), lr=lrate)
optim_discriminator = torch.optim.Adam(model.discriminator.parameters(), lr=lrate)
...
for ep in range(n_epoch+1):
    ...
    optim_generation.param_groups[0]['lr'] = lrate * (1 - ep/n_epoch)
    optim_discriminator.param_groups[0]['lr'] = lrate * (1 - ep/n_epoch)
    ...
    for im, clim, lc, _, _, _ in tqdm(train_dl, desc=f"Training epoch {ep}"):
        ...
        # Generation loss
        optim_generation.zero_grad()
        im_hat, _ = model(im, c)
        d_fake = model.discriminate(im_hat)
        generation_loss = 0.01 * F.mse_loss(d_fake, torch.zeros_like(d_fake))
        generation_loss.backward()
        optim_generation.step()
        ...
        im_hat, _ = model(im, c)
        d_real, d_fake = model.discriminate(im), model.discriminate(im_hat)
        discriminator_loss = 0.01 * 0.5 * (F.mse_loss(d_real, torch.zeros_like(d_real)) + F.mse_loss(d_fake, torch.ones_like(d_fake)))
