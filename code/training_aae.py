...
# General settings
run_name = "aae_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
n_epoch = 500
batch_size = 128
n_channels = 4
n_clim = 21
...
# Model stuff
model = AAE(n_channels=n_channels, n_latent=n_latent, n_c=[n_clim, n_lc], normalization="MCBN").to(device)
...
optim_encoder = torch.optim.Adam(model.encoder.parameters(), lr=lrate)
...
for ep in range(n_epoch+1):
    ...
    optim_encoder.param_groups[0]['lr'] = lrate * (1 - ep/n_epoch)
    ...
    for im, clim, lc, _, _, _ in tqdm(train_dl, desc=f"Training epoch {ep}"):
        ...
        encoder_loss = 0.01 * F.mse_loss(d_fake, torch.zeros_like(d_fake))
        encoder_loss.backward()
        optim_encoder.step()
        
        # Discriminator loss
        optim_discriminator.zero_grad()
        z_real = torch.randn(im.shape[0], n_latent).to(device)
        z_fake = model.encode(im, c)
        d_real, d_fake = model.discriminate(z_real), model.discriminate(z_fake)
